...
class XYConverterApp(QMainWindow):
    ...
    def get_table_data(self):
        try:
            ...
            # Itera na tabela da interface e adiciona as colunas ao DataFrame
    ...
    def check_table_data(self):
        try:
            # Pega os dados da tabela da interface e armazena em um DataFrame
            self.interfaceDF = self.get_table_data()
            fields = self.interfaceDF.columns.to_list()

            # Pega o tipo de SRC de entrada selecionado na interface
            crs_type = self.inputCRStype_cbx.currentText()
            ...
            rows_ok = True if (
                    self.interfaceDF[fields[0]].dropna().size ==
                    self.interfaceDF[fields[1]].dropna().size) \
                else False
    ...
    def convert_coordinates(self, input_y, input_x, input_EPSG, output_EPSG,
        ...
        if len(input_y) == len(input_x):
            ...
            raise Exception('There\'s an uneven number of Y and X coordinates')
        ...
        try:
            ...
            if filePath != '':
                if filePath.endswith('.csv'):
                    DF.to_csv(filePath, sep=';', decimal=',', index=True)
                else:
